...
contract GasPriceFeesHook is BaseHook {
    ...
    uint128 public s_movingAverageGasPrice;
    ...
    uint104 public s_movingAverageGasPriceCount;
    ...
    function getFee() internal view returns (uint24) {
        ...
        // if gasPrice > s_movingAverageGasPrice * 1.1, then half the fees
        if (gasPrice > (s_movingAverageGasPrice * 11) / 10) {
            return BASE_FEE / 2;
        }

        // if gasPrice < s_movingAverageGasPrice * 0.9, then double the fees
        if (gasPrice < (s_movingAverageGasPrice * 9) / 10) {
            return BASE_FEE * 2;
        }
    ...
    function _updateMovingAverage() internal {
        ...
        s_movingAverageGasPrice =
            ((s_movingAverageGasPrice * s_movingAverageGasPriceCount) +
                gasPrice) /
            (s_movingAverageGasPriceCount + 1);

        s_movingAverageGasPriceCount++;
    }
}
